...
public class OrdenaPilhaInvertido {
    ...
    public static class Pilha {
		...
		public int[] retornaMaiorComIndice(int idx) {
			Pilha aux1 = new Pilha(idx+1);
			int count = 0;
			int maior = getTopo();
			int ind = 0;
    ...
    public static void main(String[] args) {
        ...
        Pilha aux = new Pilha(n);
		Pilha pilha = new Pilha(n);

        for(int i = 0; i < n; i++) {
            aux.push(Integer.parseInt(lista[i]));
		}
		
		int count = n - 1;
		for(int i = 0; i < n; i++) {
			int[] info = aux.retornaMaiorComIndice(count--);
			pilha.push(info[0]);
			aux.inverteAteIndice(info[1]);
			aux.pop();
		}
		System.out.println("-");
